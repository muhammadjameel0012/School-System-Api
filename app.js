...
// Helmet with CSP disabled so Swagger UI and its requests work (avoids request timeout in browser)
...
// API documentation (Swagger UI) - not rate limited
app.use('/api-docs', swaggerUi.serve, swaggerUi.setup(swaggerSpec, { explorer: true }));

// Request timeout so Swagger and clients get a response instead of hanging (e.g. if DB is slow)
const API_TIMEOUT_MS = 30000;
app.use('/api', (req, res, next) => {
  const timeout = setTimeout(() => {
    if (!res.headersSent) {
      sendError(res, 504, 'Request timeout', null);
    }
  }, API_TIMEOUT_MS);
